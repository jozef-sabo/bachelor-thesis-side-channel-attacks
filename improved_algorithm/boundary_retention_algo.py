...
def improved_algorithm_step2(
        ...
        d_star: int,
        params: AttackParams,
        rs_with_same_prob_at_border: Literal["keep", "remove", "not_touch"] = "not_touch",
        three_fourths: bool = False,
    ...
    for ell, row in enumerate(Lr):
        ...
        # Deterministic tie-breaker: higher score first, then lower candidate value.
        best_sorted = sorted(((p, cand) for cand, p in scored.items()), key=lambda x: (-x[0], x[1]))

        keep_diff = 0

        if rs_with_same_prob_at_border == "keep":
            if len(best_sorted) > L:
                bordering = best_sorted[L-1]
                bordering_same_prob = [item for item in best_sorted[L:] if item[0] == bordering[0]]
                keep_diff = len(bordering_same_prob)
        elif rs_with_same_prob_at_border == "remove":
            if len(best_sorted) > L:
                bordering = best_sorted[L-1]
                bordering_same_prob = [item for item in best_sorted[:L] if item[0] == bordering[0]]
                keep_diff = -len(bordering_same_prob)
        ...
        if three_fourths:
            three_quarters_of_L = (L * 3) // 4
            if keep_diff > three_quarters_of_L:
                keep_diff = three_quarters_of_L

        best = best_sorted[:L + keep_diff]
...
def benchmark(
    ...
    for iteration in range(1, R + 1):
        ...
        Lr = improved_algorithm_step2(iteration, L, noisy_scalars, Lr, d_star, params, rs_with_same_prob_at_border, three_fourths)
        d_prefix = d_star
    return R
...
def run_dataset(
    ...
    return benchmark(blinded_with_errors[:N], d, R, L, t, params, rs_with_same_prob_at_border, three_fourths)
...
def main():
    ...
    L = 32
    t = 16
    # boundary retention strategy
    rs_with_same_prob_at_border: Literal["keep", "remove", "not_touch"] = "not_touch"
    three_fourths = False
    print(run_dataset(file_path, N=N, L=L, t=t, rs_with_same_prob_at_border=rs_with_same_prob_at_border, three_fourths=three_fourths))
